...
export default function Home() {
  ...
  const loadData = async () => {
    setLoading(true);
    const [cData, eqData] = await Promise.all([
      fetchContractors(),
      fetchEquipment(),
    ]);
  ...
  const contractorRows: ContractorDetailRow[] = contractors.map((c, idx) => {
    ...
    const baseRatios = [0.20, 0.22, 0.21, 0.24, 0.23, 0.26, 0.25, 0.27, 0.28, 0.29];
  ...
  const overallRatio = contractorRows.length > 0
    ? contractorRows.reduce((acc, curr) => acc + curr.fuelRatio, 0) / contractorRows.length
    : 0.24;

  const topContractor = contractorRows.length > 0
    ? [...contractorRows].sort((a, b) => a.fuelRatio - b.fuelRatio)[0].name
    : "PT. A";
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/90 border border-slate-800 mb-6 shadow-xl backdrop-blur-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Pilih kontraktor spesifik untuk memfilter metrik & laporan</p>
                </div>
              </div>
            ...
            <ContractorEquipmentFleet
              ...
              selectedContractorName={
                selectedContractor === "ALL"
                  ...
                  : contractorRows.find((c) => c.code === selectedContractor)?.name || selectedContractor
              }
              equipments={
                selectedContractor === "ALL"
                  ? equipments
        ...
        {activeFeature === 2 && (
          <div className="p-12 rounded-2xl bg-slate-900/60 border border-slate-800 text-center max-w-xl mx-auto my-16">
            ...
            <h2 className="text-xl font-bold text-white">Feature 2: Monitoring Konsumsi Fuel Berbasis Aktivitas</h2>
            <p className="text-xs text-slate-400 mt-2">
              Modul ini akan difokuskan pada pemantauan rinci berbasis volume, BCM, dan working hours pada fase pengembangan selanjutnya.
            </p>
          </div>
        )}
        ...
        {activeFeature === 3 && (
          <div className="p-12 rounded-2xl bg-slate-900/60 border border-slate-800 text-center max-w-xl mx-auto my-16">
            ...
            <h2 className="text-xl font-bold text-white">Feature 3: Penyelarasan Fuel Ratio dengan SPO & Target</h2>
            <p className="text-xs text-slate-400 mt-2">
              Modul ini akan difokuskan pada evaluasi otomatis terhadap Standar Parameter Operasi dan Target Produksi.
            </p>
          </div>
        )}
      </main>
